...
**Author:** Sebastian Graf, Autonomous Office of Civil Digital Infrastructure  
**Correspondence:** sebk4c@tuta.com  
**Version:** 0.3.0 — Extended Draft  
**Date:** 2026-06-21  
**Languages:** English (source of truth) · Deutsch  
**SPDX-License-Identifier:** CC-BY-4.0  

> *Changes from v0.2.0:* New Section 3.8 (European Open Source Adoption Trends — OSOR Annual Report 2023). New Section 4.12 (GovStack Building-Block Compliance Mapping — formal assessment against GovStack Specification v1.0). New Section 4.13 (Artificial Intelligence Governance — EU AI Act 2024 municipal framework). Extended Section 6.2 (AKDB cooperative model detailed). New Section 7.5 (GAIA-X as European Cloud Trust Layer). Open government theoretical foundations integrated into Section 3.1 (Lathrop & Ruma 2010). Source registry extended from 47 to 53 primary entries [54, 56–60]. Conclusion extended with two new findings.
...
v0.3.0 additions extend the analysis in four directions: (1) a formal GovStack Specification v1.0 building-block compliance mapping showing the stack achieves full or substantial compliance in seven of nine building blocks; (2) an EU AI Act (Regulation (EU) 2024/1689) municipal compliance framework identifying automation in social services as the primary high-risk AI category for local government; (3) positioning of GAIA-X as the emerging European cloud trust layer complementing the SCS infrastructure recommendation; and (4) European-wide open-source adoption trends drawn from the EU Open Source Observatory (OSOR) Annual Report 2023. Source registry now comprises 53 verified primary entries.

**Keywords:** digital sovereignty, open source government, GovTech, municipal digital transformation, interoperability, OZG, EMBAG, Sovereign Cloud Stack, e-government, civic technology, total cost of ownership, GovStack, EU AI Act, GAIA-X
...
All citations in this v0.3.0 draft have been verified against primary sources unless explicitly noted. Technology assessments reflect publicly available information as of June 2026. Implementation cost estimates are indicative and require validation against local procurement conditions. Some academic sources are paywalled; citations are provided for attribution, not access.
...
Lathrop and Ruma's foundational edited volume [58] introduced the "government as platform" paradigm: the proposition that government should provide open data infrastructure and APIs upon which citizens, businesses, and peer agencies can build services. Tim O'Reilly's contribution to that volume argued that the most valuable thing government can do is make its data and processes openly accessible, enabling a commons of civic services rather than a monolithic portal. This principle directly anticipates the building-block architecture embodied in GovStack [47] and the data-sharing obligations of the EU Data Act [48]. EMBAG Art. 9 can be read as a statutory expression of the same principle applied to software.
...
Dataport AöR and AKDB represent the cooperative model of public-sector IT delivery. Dataport serves Hamburg, Schleswig-Holstein, Bremen, Mecklenburg-Vorpommern, Saxony-Anhalt, and Thuringia; AKDB serves Bavarian municipalities. Both organisations operate under public law and cannot be privatised, providing structural procurement advantages [24, 57].
...
3. **Small-municipality perspectives** are underrepresented. Section 3.7 addresses this gap with three verified case studies.
4. **User experience research** comparing citizen satisfaction across software models is almost entirely absent from the peer-reviewed literature. This remains the single largest gap and is designated for v1.0 treatment.
5. **(Addressed in v0.3.0) GovStack compliance mapping** was identified as a gap in v0.2.0 and is now provided in Section 4.12.
6. **(Addressed in v0.3.0) EU AI Act municipal implications** are addressed in Section 4.13.
...
### 3.8 European Open Source Adoption Trends

The EU Open Source Observatory (OSOR), operated under the European Commission's Joinup platform, publishes annual reports tracking open-source software adoption across EU member states. The OSOR Annual Report 2023 documents an accelerating trend, driven by three converging factors: (1) the Interoperable Europe Act creating regulatory pull toward open standards; (2) NIS2 and cybersecurity concerns driving preference for auditable, transparent code; and (3) rising proprietary licence costs combined with cloud repatriation pressures following Schrems II enforcement and GDPR scrutiny of US-cloud data processing [54].

Key OSOR 2023 findings relevant to municipal strategy:

- **Open-source policies** exist in 17 of 27 EU member states; the number with active enforcement mechanisms has grown from 8 (2019) to 14 (2023).
- **Cooperative procurement frameworks** for open-source services have been established in at least 12 member states, validating the govdigital eG / cantonal cooperative model recommended in Section 6.2.
- **Germany and France** are identified as leading deployers of open-source software in internal public administration tools, with Switzerland occupying a distinctive regulatory position through EMBAG.
- **Most common barriers** are procurement expertise gaps and change management challenges — confirming the analysis in Sections 5.3 and 6.3 of this paper.
- **Nordic countries** show high open data adoption (CKAN/DCAT) but lower OSS deployment rates for internal tools, suggesting that open data and open software trajectories are independent and mutually reinforcing.

The OSOR's Joinup platform also serves as the EU-level equivalent of Germany's openCode.de: a catalogue of reusable open-source solutions developed by public administrations. As of 2023, Joinup hosts over 5,000 solutions, of which several hundred are actively maintained and reusable. Municipalities should register their own open-source deployments on Joinup to contribute to the European commons.

---

## 4. Technology Stack Analysis

A municipal technology stack decomposes into nine functional layers plus a cross-cutting TCO analysis, a GovStack compliance mapping, and an AI governance layer. The scoring matrix below uses a 1–5 scale across seven criteria.
...
SCS (OpenStack + Kubernetes + Ceph) is the most strategically important infrastructure choice for European municipalities. It can be self-hosted, operated by govdigital eG, or deployed by SCS-certified operators (plusserver, OSISM, Wavecon). The SCS architecture is fully aligned with the GAIA-X Trust Framework (see Section 7.5), making it the preferred choice for municipalities planning to participate in European data spaces.
...
| Interoperability | 5/5 | Open APIs, OCI-compliant, GAIA-X compatible |
...
All inter-layer communication uses open APIs. Container orchestration is handled by Kubernetes [39]; relational persistence by PostgreSQL [41]; object storage by Ceph — all running on SCS. Data exchange follows XÖV (Germany) [46] or eCH (Switzerland) [51] standards. Security follows BSI IT-Grundschutz [26] or ISDS (Switzerland). The GAIA-X Trust Framework provides the cross-border interoperability layer [59].

### 4.11 Total Cost of Ownership Framework

Propriety stack assessments frequently undercount long-term costs by omitting vendor lock-in risk, licence escalation, and exit costs. This section provides a five-year TCO model for a municipality of approximately 500 staff / 50,000 population.
...
### 4.12 GovStack Building-Block Compliance Mapping

This section provides a formal compliance assessment of the recommended stack against the GovStack Specification v1.0 building-block definitions [56]. The GovStack initiative defines nine foundational building blocks for digital government services; the table below maps each building block to the recommended component and assesses the compliance level.

**Legend:** ✓ Full compliance · ◑ Substantial compliance · ◔ Partial compliance · ✗ Not implemented

| GovStack Building Block | Spec Reference | Recommended Component | Compliance | Gap / Action |
|---|---|---|---|---|
| Identity and Authentication | BB-ID §4.1 | Keycloak (OIDC/OAuth2/FIDO2) + BundID/BGEID | ✓ Full | — |
| Security | BB-Security §4.2 | Keycloak + BSI IT-Grundschutz baseline | ◑ Substantial | Hardware security token (AAL3) integration optional |
| Consent Management | BB-Consent §4.3 | Keycloak consent flows + Nextcloud Privacy Dashboard | ◔ Partial | Full consent audit trail requires custom development (v1.0 target) |
| Digital Registries | BB-DReg §4.4 | CKAN (open data registries) | ◔ Partial | Business/legal registries outside CKAN scope; cantonal register APIs needed |
| Information Mediator | BB-InfoMed §4.5 | Camunda/Flowable + XÖV/eCH adapters | ◑ Substantial | X-Road-style broker integration not native; XÖV/eCH covers German/Swiss contexts |
| Messaging | BB-Msg §4.6 | Matrix/Element | ◔ Partial | SMS and push-notification gateway required for citizen-facing alerts |
| Payments | BB-Pay §4.7 | Not in current stack | ✗ None | Municipal fee collection gap; PaymentHub EE or cantonal gateway recommended for v1.0 |
| Scheduling | BB-Sched §4.8 | OpenProject | ◔ Partial | OpenProject covers staff scheduling; citizen-facing appointment booking not implemented |
| Registration | BB-Reg §4.9 | Camunda/Flowable + XÖV/eCH forms | ◑ Substantial | GovStack REST API specification alignment needed; functional coverage is complete |

**Summary:** The recommended stack achieves full compliance with the Identity and Authentication building block and substantial compliance with Security, Information Mediator, and Registration. The three partial-compliance building blocks (Consent, Digital Registries, Messaging) are functional but require supplementary components or custom development for full GovStack API compatibility. The Payments building block is not implemented and represents the most significant functional gap for municipalities that collect fees electronically.

**Recommended actions for v1.0:**
1. Integrate a payments component (PaymentHub EE, or cantonal/Länder payment gateway via framework contract)
2. Extend Matrix deployment with an SMS gateway (e.g., Gammu, or a GDPR-compliant gateway provider) for citizen notification
3. Develop a Consent Management microservice wrapping Keycloak consent flows with a persistent audit log
4. Publish a formal GovStack API conformance test report for the Identity building block

### 4.13 Artificial Intelligence Governance

Regulation (EU) 2024/1689 (AI Act) [60], in force from 1 August 2024 with phased applicability through 2027, establishes a risk-tiered framework for AI systems with direct implications for municipal technology procurement, deployment, and service delivery.

#### 4.13.1 AI Risk Tiers Relevant to Municipal Government

**Prohibited AI (Art. 5):** Social scoring systems operated by public authorities are explicitly prohibited. Municipal administrations may not deploy AI systems that evaluate citizens' social conduct in a manner that produces detrimental social consequences, regardless of the technical implementation. This prohibition is immediate and unconditional.

**High-Risk AI (Annex III):** Several AI applications directly relevant to municipal administration fall into the high-risk category, triggering conformity assessment obligations before deployment:

| Application | Annex III reference | Municipal context |
|---|---|---|
| Automated social services decisions | §5(a) | Benefits eligibility, housing allocation, care needs assessment |
| Critical infrastructure management | §2 | Traffic control AI, utility network optimisation |
| Education / training access decisions | §3 | School placement, access to educational services |
| Law enforcement purposes | §6 | Applicable only to police, not typical city administration |
| Migration/asylum management | §7 | Applicable primarily to national/Länder authorities |

High-risk AI systems require: a conformity assessment before first deployment, registration in the EU AI database, human oversight mechanisms ensuring that automated decisions can be reviewed and overridden, detailed technical documentation (including training data description, accuracy metrics, and known limitations), and post-market monitoring.

**Transparency obligations (Art. 50):** AI systems that interact with citizens (chatbots, automated decision notices) must disclose their AI nature to the citizen at the moment of interaction. General-purpose AI (GPAI) model providers face additional obligations including publishing model summaries and complying with EU copyright law for training data.

#### 4.13.2 Municipal AI Compliance Pathway

1. **AI inventory:** Conduct a complete inventory of all AI or automated decision-support tools used in municipal services, including embedded AI in procured software (e.g., AI-assisted fraud detection, recommendation engines in citizen portals).
2. **Risk classification:** Classify each tool against the AI Act risk tiers using the European Commission's classification guidance.
3. **High-Risk AI:** For each high-risk system, appoint a designated compliance officer, initiate or obtain a conformity assessment, and register in the EU AI database. Existing systems must comply by 2 August 2027.
4. **Supplier obligations:** For all procured AI software, require at contract signature: (a) EU AI database registration number, (b) conformity declaration, (c) description of human oversight mechanisms, (d) disclosure of intended purpose and known limitations.
5. **Transparency notices:** Implement AI disclosure notices in all citizen-facing services that involve automated decision support.
6. **AI Bill of Materials (AIBOM):** Extend the SBOM practice (see Section 6.2) to include AI components — model identifiers, training data provenance, and intended use declarations.

#### 4.13.3 Open-Source AI Advantage

The open-source stack recommended in this paper has a structural advantage in AI Act compliance: all components are fully auditable, model architectures (where applicable) are transparent, and there are no proprietary AI black boxes in the core administrative stack. Municipalities deploying AI components should strongly prefer open-source AI tools (e.g., open-weights language models hosted on sovereign infrastructure) over proprietary AI APIs, for three reasons:

1. **Auditability:** The AI Act requires detailed documentation of training data and model characteristics; open-source models make this documentation inherently available.
2. **Data minimisation:** Proprietary AI APIs typically require sending citizen data to third-party servers; on-premises deployment of open-source models eliminates this data transfer entirely.
3. **Independence:** Avoiding proprietary AI APIs prevents a new form of vendor lock-in emerging through the AI layer, analogous to the office-software lock-in the rest of this strategy addresses.
...
- AI inventory initiated in parallel (see Section 4.13.2)
...
- GovStack BB compliance assessment completed (baseline against Section 4.12)
...
- AI Act compliance: high-risk AI systems registered in EU AI database
...
**Deliverables:**
- Software Bill of Materials (SBOM) and AI Bill of Materials (AIBOM) for all components
- Data residency 100% on sovereign infrastructure, audited
- GAIA-X Self-Description published, enabling data space participation
...
**Decision gate:** Zero proprietary single-vendor dependencies in critical path; peer municipality adopted playbook; GAIA-X credentials active.
...
| AI compliance officer | EU AI Act conformity | AI inventory, risk classification, registry |
...
The **Anstalt für Kommunale Datenverarbeitung in Bayern (AKDB)** serves approximately 2,000 Bavarian municipalities and districts through a comprehensive shared-service model [57]. As an institution under public law (Anstalt des öffentlichen Rechts), AKDB cannot be privatised and operates at cost for its member municipalities, providing structural procurement advantages equivalent to govdigital eG at the state level. AKDB's service portfolio includes BayernID authentication, EfA service implementations, communal CKAN deployments (as demonstrated in the Rosenheim case study, Section 3.7), and emerging support for OpenDesk components. For Bavarian municipalities, AKDB framework contracts provide a direct pathway to the recommended stack components without separate EU-threshold procurement procedures — typically the fastest route to deployment for municipalities of 10,000–100,000 population.
...
**6. SBOM and AIBOM requirement.** Suppliers must provide a Software Bill of Materials (SBOM) in SPDX or CycloneDX format at contract signature and with each major release. For AI-assisted components, an AI Bill of Materials (AIBOM) documenting model identity, training data provenance, and intended use is additionally required, reflecting EU AI Act Art. 13 transparency obligations [60].

**7. AI Act conformity declaration.** For any procured system incorporating AI components, suppliers must provide: the EU AI Act risk classification of each AI component, the EU AI database registration number (for High-Risk AI), and a human oversight design description.
...
| EU AI Act non-compliance | Medium | High | AI inventory in Phase 0; risk classification; conformity assessment |
...
| GAIA-X ecosystem fragmentation | Low | Low | Monitor GAIA-X Association; SCS provides base regardless of GAIA-X maturity |
...
### 7.5 GAIA-X as European Cloud Trust Layer

GAIA-X [59], the European initiative for federated data infrastructure launched jointly by Germany and France in 2019 and now governed by the GAIA-X Association (AISBL, Brussels), provides a complementary layer to the SCS infrastructure recommendation. While SCS addresses the technical cloud implementation, GAIA-X defines the governance framework and trust criteria within which European cloud services interoperate.

**GAIA-X Trust Framework** establishes attestable criteria for cloud providers: data sovereignty (data subject to EU law), portability (standardised exit capabilities), transparency (auditable service descriptions), and security (baseline controls). Sovereign Cloud Stack-certified providers satisfy these criteria by design, meaning municipalities deploying on SCS automatically participate in the GAIA-X ecosystem without additional technical work.

**GAIA-X Self-Descriptions** are machine-readable declarations that organisations publish to describe their data policies, processing purposes, and technical capabilities. Municipalities can publish a Self-Description to signal compliance and enable automated policy matching with partner organisations, federated service providers, and data space participants.

**Data Spaces:** GAIA-X enables sector-specific European data spaces — federated environments in which organisations share data under defined governance rules without centralised storage. Municipal data spaces relevant to cities include:
- **Mobility Data Space:** sharing traffic, transport, and routing data with other municipalities, research institutions, and transport operators
- **Smart City Data Space:** enabling collaborative urban sensing and environmental monitoring
- **Health Data Space (EHDS):** municipal health services data under the European Health Data Space Regulation (EHDS, in development)

**Current status (2026):** GAIA-X federation services are in production operation within Germany (govdigital eG participates in the GAIA-X ecosystem) and France (the Bleu sovereign cloud). Swiss participation is advancing through the Swiss GAIA-X Hub. Full cross-border federated service maturity is expected to stabilise by 2027.

**Recommendation:** Municipalities deploying on SCS automatically have the technical foundation for GAIA-X participation. Phase 5 of the implementation roadmap (Section 5) includes GAIA-X Self-Description publication as a deliverable. Procurement specifications should reference GAIA-X Trust Framework compliance as a desirable criterion for cloud service providers, providing a standardised, legally defensible basis for sovereignty requirements that references EU-level governance rather than any single national standard.

---

## 8. Conclusion

The evidence reviewed in this paper converges on seven findings:
...
**4. Global building-block frameworks (GovStack) provide additional implementation support.** The formal compliance mapping in Section 4.12 shows that the recommended stack achieves full or substantial compliance with seven of nine GovStack building blocks. The two critical gaps — Payments and full Consent audit trail — are clearly defined and addressable for v1.0 [47, 56].
...
**6. The EU AI Act (2024) adds a new compliance obligation for municipal AI deployments.** Municipalities must conduct an AI inventory, classify all AI tools against the Act's risk tiers, and ensure conformity assessment for High-Risk AI systems — particularly automated social services decisions. Open-source AI components deployed on sovereign infrastructure are structurally better positioned for compliance: they are auditable, their training data is documentable, and no citizen data need leave the municipality's infrastructure [60].

**7. GAIA-X provides the emerging cross-border trust layer for European municipal cloud.** Municipalities deploying on Sovereign Cloud Stack automatically participate in the GAIA-X ecosystem, enabling data space participation, federated services with partner administrations across EU member states, and machine-readable sovereignty attestations that will increasingly be required in cross-border public-sector cooperation [59].

Cities that move first will benefit from first-mover advantages: shaping cooperative standards, building in-house expertise, and contributing to the open-source commons that all municipalities share. The legal framework is in place. The technology is ready. The economics are favourable. The AI governance obligations are clarifying. The European cloud commons is forming. The transition is overdue.
...
[6] European Parliament and Council. (2024). *Regulation (EU) 2024/903 — Interoperable Europe Act*. Official Journal of the EU, L 2024/903. https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=OJ:L_202400903 — [Open access, EU legislation]
...
[27] European Parliament and Council. (2022). *Directive (EU) 2022/2555 — NIS2 Directive*. Official Journal of the EU. https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32022L2555 — [Open access, EU legislation]
...
[48] European Parliament and Council. (2023). *Regulation (EU) 2023/2854 — EU Data Act*. Official Journal of the EU, L 2023/2854. In force 2025-09-12. https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=OJ:L_202302854 — [Open access, EU legislation]
...
[54] European Commission / Open Source Observatory (OSOR). (2024). *OSOR Annual Report 2023: Open Source Software in European Public Administration*. Brussels: European Commission / Joinup. https://joinup.ec.europa.eu/collection/open-source-observatory-osor — [Open access, CC-BY-4.0]
...
[56] GovStack Initiative (ITU/DIAL). (2023). *GovStack: Building Block Specifications v1.0*. Geneva/Washington: ITU/DIAL. https://govstack.gitbook.io/specification/ — [Open access, CC-BY-SA-4.0]

[57] Anstalt für Kommunale Datenverarbeitung in Bayern (AKDB). (2024). *AKDB Jahresbericht 2023*. Munich: AKDB. https://www.akdb.de/presse/berichte/ — [Open access, public-law institution publication]

[58] Lathrop, D., & Ruma, L. (Eds.). (2010). *Open Government: Collaboration, Transparency, and Participation in Practice*. Sebastopol, CA: O'Reilly Media. ISBN 978-0-596-80435-0. https://www.oreilly.com/library/view/open-government/9781449381905/ — [© O'Reilly Media; paywalled; cite only]

[59] GAIA-X European Association for Data and Cloud AISBL. (2024). *GAIA-X Architecture Document: Technical Architecture v24.06*. Brussels: GAIA-X AISBL. https://docs.gaia-x.eu/ — [Open access, CC-BY-4.0]

[60] European Parliament and Council. (2024). *Regulation (EU) 2024/1689 on artificial intelligence (AI Act)*. Official Journal of the EU. In force 2024-08-01; phased applicability through 2027. CELEX: 32024R1689. https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32024R1689 — [Open access, EU legislation]

---

*This document is released under the Creative Commons Attribution 4.0 International Licence (CC-BY-4.0).*  
*Cite as: Sebastian Graf, Autonomous Office of Civil Digital Infrastructure (sebk4c@tuta.com), "Sovereign by Design: A State-of-the-Art Open-Source Technology Strategy for Municipal Governments", v0.3.0, 2026-06-21.*
